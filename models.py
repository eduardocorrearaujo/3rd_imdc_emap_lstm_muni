...
def sum_regions_predictions(model, df, enso, test_year, columns_to_normalize, boxcox = False, n_passes = 500):
    ...
    for geo in df.geocode.unique():

        df_w = prep.aggregate_data(df, geo, column = 'geocode')
...
def apply_geocode_predictions(model, df, enso, test_year, columns_to_normalize, boxcox = False, n_passes = 500):
    '''
    Função que aplica o modelo para todas as regionais de saúde e retorna a soma,
    que representa a função para o estado no formato de um dataframe. Não sei se existem formas de
    otimizar esse loop for. 
    ...
    if len(df.geocode.unique()) > 1:
        raise("The df input must have a single geocode")

    df = prep.add_epiweek_label(df)
        
    if boxcox: 
        df['casos']= fn_boxcox(df['casos'] + 1, THR)

    data = df.merge(enso[indicators], left_index = True, right_index = True)
    ...
    predicted = evaluate_samples(model, X_test, n_passes=n_passes)
        
    predicted = predicted*norm_values['casos']

    if boxcox: 
        predicted = inv_boxcox(predicted.numpy(), THR) -1 
